...
当前版本聚焦稳定、本地、可测试、可演示的 GitHub 展示版。不依赖 OpenAI API、数据库、登录系统或部署平台。DeepSeek / LLM 报告增强是可选能力，默认关闭，不影响主流程验收。v0.4 增加了轻量 Data Quality Checks 和 Streamlit 内置图表展示；v0.5 增加了默认关闭的 experimental LangGraph orchestration adapter，用于展示多 Agent 工作流工程能力。
